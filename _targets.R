...
list(
  ...
  tar_target(
    Temperatures_XGBoost_summer_avgs,
  ...
  tar_target(
    Tract_RaceEthn_Census,
    get_Census_tract_data(RUCAs_2000, RUCAs_2010)
  ),
  tar_target(
    race_hour_temps_NEMIA_metro,
      map_dfr(all_years, ~create_weighted_race_hours_NEMIA(.x, urban = "metropolitan", Tract_RaceEthn_Census), .progress = T) %>%
      tar_group(),
    iteration = "group"
  )
)
